...
const AbuseReportDetails: React.FC<AbuseReportDetailsProps> = ({ onBack, reportType, closeModal }) => {
  ...
  return isSubmitted ? (
    <div className="m-auto flex items-center justify-center text-center flex-col gap-4 self-center h-fit px-4">
      <CheckCircle className="w-20 h-20 text-green-500" /> {/* ✅ tick icon */}
      <h2 className="font-semibold text-3xl">شكرًا لك!</h2>
      <p className="text-gray-500">تم إرسال طلبك وهو في الطريق. تحقق من بريدك الإلكتروني للحصول على التفاصيل.</p>
      <MainButton className="mt-4" text="استمرار" onClick={() => closeModal()} /> {/* ✅ close modal */}
    </div>
  ) : (
    <motion.div
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-4">
            <FormInput type="text" placeholder="اكتب هنا" label="الموضوع" name="subject" />
            <FormInput area placeholder="اكتب هنا" label="المحتوى/الفرع" name="content" />
          </div>
